...
class Observer:
    ...
    def _get_obs_layers(self) -> Dict[str, Dict[int, str]]:
        ...
        def _register_layer(
            info_dict: Dict[int, str], contents: List[str], start: int = 0
        ):
        ...
        ingredient_channels = [
            f"ingredient{n}" for n in range(self.layout.num_ingredients)
        ]
        ...
        for i in range(max(self.capacity)):
            _register_layer(
                self_obs_layers,
                [f"({i}) {inv}" for inv in inventory_channels],
                len(self_obs_layers),
            )
        ...
        for i in range(max(self.capacity)):
            _register_layer(
                other_obs_layers,
                [f"({i}) {inv}" for inv in inventory_channels],
                base_count + len(other_obs_layers),
            )
        layer_infos["other"] = other_obs_layers

        object_channels = (
            ["plate", "cooked", "used", "dirt"] + ingredient_channels + ["count"]
        )
        order_max = self.config.parameter.order_max
        order_channels = sum(
            [
                [f"order{n}(0)", f"order{n}(1)", f"order{n}(2)"]
    ...
    def observe_context(self, state: State):
        ...
        def next_reservation_time():
            ...
            return jnp.min(
                time_to_arrival, initial=1000, where=time_to_arrival >= 0
            ).astype(int)

        next_reservation_time = jax.lax.cond(
            reservation_remaining, next_reservation_time, lambda: -1
        )
        context = jnp.array(
            [
                state.time,
    ...
    def observe_line(self, state: State):
        ...
        entrance_pos = jnp.array(self.layout.entrance_positions).squeeze()
        length_info = jnp.array(
            [state.line.reserved_line_length, state.line.line_length]
        )
    ...
    def observe_customer(self, state: State):
        ...
        def _observe_recipe(obj):
            ...
            return jax.lax.cond(
                obj > 0,
                DynamicObject.get_ingredient_idx_list_jit,
                lambda _: jnp.full((3,), -1),
                obj,
            )
        ...
        def _food_finished(food):
            ...
            return jax.lax.cond(
                food == DynamicObject.USED | DynamicObject.PLATE,
                lambda: 1,
                lambda: 0,
            )
        ...
        customer_features = jnp.concat(
            [status, flatten_recipes, finished_plates], axis=1
        )
        customer_layers = jnp.zeros(
            (self.height, self.width, customer_features.shape[1])
        )
        ...
        customer_layers, _ = jax.lax.scan(
            _set_feature, customer_layers, (table_pos, customer_features)
        )
    ...
    def get_obs(self, state: State) -> Tuple[jax.Array, jax.Array]:
        ...
        def _agent_layers(agent, storage):
            ...
            pos_layers = (
                jnp.zeros((height, width, 1), dtype=jnp.int32).at[*pos, 0].set(1)
            )
            dir_layers = (
                jnp.zeros((height, width, 2), dtype=jnp.int32).at[*pos].set(direction)
            )
            # 最後に見たステップ数
            observe_step_layer = jnp.expand_dims(agent.grid_observed_step, -1)
            # 最後に見てからのステップ数
            freshness_layer = (
                jnp.full_like(observe_step_layer, state.time) - observe_step_layer
            )
            ...
            init_inv_grid = jnp.zeros(
                (*ingredients.shape, obj_layers * max(self.capacity))
            ).reshape(*ingredients.shape, -1, obj_layers)
            inv_layers = jax.lax.fori_loop(
                0,
                storage,
                _obs_inventory,
                init_inv_grid,
        ...
        def _agent_obs(agent_id: jnp.ndarray):
            agent_layers = jax.vmap(_agent_layers)(
                state.agents, jnp.array(self.capacity)
            )
            ...
            # 視野範囲内に観測情報を制限
            return jax.lax.cond(
                self.config.parameter.restrict_observation,
                lambda: jnp.where(mask, raw_obs, -1),
                lambda: raw_obs,
            )
