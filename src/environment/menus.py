...
class MenuList(PyTreeNode):
    ...
    def correct(self, dish, ordered_menus: jnp.ndarray):
        def _get_orders(i, ordered_menu_recipes):
            return jax.lax.cond(
                ordered_menus[i] >= 0,
                lambda: ordered_menu_recipes.at[i].set(jnp.sort(self.menu[ordered_menus[i]])),
                lambda: ordered_menu_recipes.at[i].set(-1),
            )

        order_nums = ordered_menus.shape[0]
        correct_recipes = jax.lax.fori_loop(0, order_nums, _get_orders, jnp.zeros((order_nums, 3), dtype=int))
        ...
        correct_order_idx = jnp.argmax(is_correct_order)  # 正しいレシピを提供したときのみ使用する
